...
- 측정 대상은 검색 SELECT(정렬 + `LIMIT 20 OFFSET 0`). 총 건수는 `COUNT(*) OVER()`로 본 쿼리에서 함께 얻으므로 별도 COUNT 쿼리가 없다(통합 전후 비교는 § 3 하단).
...
### COUNT 단일 쿼리 통합 (코드 리뷰 M2 반영)

캐시 미스 1회가 COUNT + SELECT 2회 실행이던 것을 `COUNT(*) OVER()` 윈도 함수로 본 쿼리 1회에 통합했다. `LIMIT` 적용 전 전체 결과 기준으로 계산되므로 총 건수가 그대로 나온다. 시나리오 A 웜 기준 실측:

| 구성 | 실행 시간 (웜) |
|------|---------------|
| 통합 전: COUNT + SELECT 2회 | 125 + 124 = **249 ms** |
| 통합 후: 단일 쿼리 (`COUNT(*) OVER()` 포함) | **129 ms** (−48%) |

- 윈도 함수 자체의 오버헤드는 +5 ms 수준(SELECT 단독 124 → 129 ms). 비용의 대부분인 파생 테이블을 1회만 계산하는 이득이 지배적이다.
- 본 쿼리가 0행이면 "조건 일치 0건"과 "범위 밖 페이지"를 구분할 수 없어, 그때만 COUNT 쿼리를 폴백으로 실행한다.
